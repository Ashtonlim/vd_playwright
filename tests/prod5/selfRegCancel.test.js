...
describe('removes data', () => {
  it('clear queue in queue list', async () => {
    ...
    if (medServiceExists === false) {
      ...
      await page.fill(
        'textarea',
        'Dear {patient.name} email here is the link to self-reg-cancel {appointment.confirmPaymentLink}, repeat same link again ->>>'
      )
      ...
      await page.fill(
        'textarea',
        'Dear {patient.name} sms here is the link to self-reg-cancel {appointment.confirmPaymentLink}, repeat same link again ->>>'
      )
      ...
      await page.click('text=Patient Name')
      await page.click('textarea')
      await page.fill('textarea', 'Dear {patient.name} yr appt w to be cancelled: {appointment.confirmPaymentLink} --- ')
      await page.click('text=Add Template')
      await page.click('textarea')
      await page.fill('textarea', 'Dear {patient.name} yr appt w to be cancelled: {appointment.confirmPaymentLink} --- {appointment.service} ')
