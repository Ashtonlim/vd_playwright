...
describe('removes data', () => {
    it('clear queue in queue list', async () => {
        ...
        await page.selectOption(
            'text=Type GeneralReferralMedical Certificate >> select',
            'Referral'
        )
        ...
        await page.selectOption(
            'text=PatientTitleNameNRICGiven IDLocal Name >> select',
            '{patient.title}'
        )

        await page.selectOption(
            'css=.panel-body div > div:nth-child(5) div:nth-child(2) select',
            '{date.today_date}'
        )
        await page.selectOption(
            'css=.panel-body div > div:nth-child(5) div:nth-child(3) select',
            '{clinic.name}'
        )
        await page.selectOption(
            'css=.panel-body div > div:nth-child(5) div:nth-child(4) select',
            '{provider.name}'
        )

        await page.selectOption(
            'text=REFERRAL INFONameCategoryDescriptionAddressContact NumberFaxEmail >> select',
            '{referral.email}'
        )
        await page.selectOption(
            'text=REFERRAL INFONameCategoryDescriptionAddressContact NumberFaxEmail >> select',
        ...
        await page.selectOption(
            'text=SortCreated - Newest to OldestCreated - Oldest to NewestEdited - Newest to Oldes >> select',
            'updatedAt.-1'
        )
        ...
        await page.waitForSelector(
            'text=NameDescriptionDosage InstructionQuantityActionsmeds1 >> :nth-match(path, 2)'
        )
        await page.click(
            'text=NameDescriptionDosage InstructionQuantityActionsmeds1 >> :nth-match(path, 2)'
        )
        ...
        await page.click(
            'text=A37.00 - Whooping cough due to Bordetella pertussis without pneumonia'
        )
        await page.click(
            'text=Primary Diagnosis Secondary Diagnosis Additional Diagnosis >> :nth-match([placeholder="Record ICD 10"], 2)'
        )
        await page.fill(
            'text=Primary Diagnosis Secondary Diagnosis Additional Diagnosis >> :nth-match([placeholder="Record ICD 10"], 2)',
            'fever'
        ...
        await page.click('i')
        await page.click(
            'text=Primary Diagnosis Secondary Diagnosis Additional Diagnosis >> button'
        )
        ...
        await page.click(
            'text=27 May 2021 Visit Notes > From Date < May 2021 > SunMonTueWedThuFriSat 123456789 >> :nth-match(path, 2)'
        )
